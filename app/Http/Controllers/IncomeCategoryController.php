...
class IncomeCategoryController extends Controller
{
    ...
    public function view($id){
        $data=IncomeCategory::where('incate_status',1)->where('incate_id',$id)->firstOrFail();
    ...
    public function insert(Request $request){
        $this->validate($request,[
            ...
            'name.max'=>'Please enter 50 character only',
    ...
    public function update(Request $request){
        ...
        //for this hidden id is taken at that page
        ...
        if($update){
            Session::flash('success','Successfully updated income category information.');
            return redirect('dashboard/income/category/view/'.$id);
        }else{
            Session::flash('error','Opps! Operation Failed.');
